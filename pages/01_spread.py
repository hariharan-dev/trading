...
from calendar_spread.futures_data import (
    backtest_calendar_spread,
    get_continuous_futures_data,
)
...
def display_backtest_results(
    ...
    if not trades_df.empty:
        ...
        with col2:
            ...
            max_loss = (
                losing_trades["PnL"].min() if not losing_trades.empty else 0
            )

            # Get winning trades (PnL > 0) for min/max profits
            winning_trades = trades_df[trades_df["PnL"] > 0]
            min_profit = (
                winning_trades["PnL"].min() if not winning_trades.empty else 0
            )
            max_profit = (
                winning_trades["PnL"].max() if not winning_trades.empty else 0
            )
...
def display_stocks(stocks_subset: pd.DataFrame) -> None:
    ...
    for idx, row in stocks_subset.iterrows():
        ...
        wins_losses = ""
        if row["Symbol"] in st.session_state.backtest_results:
            trades_df = st.session_state.backtest_results[row["Symbol"]][
                "trades_df"
            ]
            if not trades_df.empty:
        ...
        with st.expander(label_content):
            if row["Symbol"] in st.session_state.backtest_results:
                ...
                display_backtest_results(
                    results["trades_df"], results["backtest_df"]
                )

                # Initialize session state for toggles if not present
                trades_key = f"show_trades_{row['Symbol']}"
                table_key = f"show_table_{row['Symbol']}"

                if trades_key not in st.session_state:
                    st.session_state[trades_key] = False
                ...
                show_trades = st.checkbox(
                    "View Trade Details (last 100 days)", key=trades_key
                )
                if show_trades:
                    st.dataframe(
                ...
                # Continuous Futures Data Section
                st.subheader("Continuous Futures Data")
                show_table = st.checkbox("View Data Table", key=table_key)
                if show_table:
                    st.dataframe(
                        results["backtest_df"],
                        hide_index=True,
...
def main():
    ...
    st.title("Calendar Spread Strategy")

    # Initialize analysis_started in session state if not present
    if "analysis_started" not in st.session_state:
        st.session_state.analysis_started = False

    # Create a container for controls
    control_container = st.container()
    col1, col2 = control_container.columns([2, 1])

    with col1:
        use_fresh_data = st.checkbox(
            "Fetch Fresh Data",
            value=False,
            help="If checked, fetches fresh data from NSE instead of using cached data. "
            "Warning: This may take longer and is subject to rate limits.",
        )

    with col2:
        if st.button(
            "Start Analysis",
            help="Click to start fetching and analyzing data",
            type="primary",
        ):
    ...
    if not st.session_state.analysis_started:
        st.info(
            """
        👆 Click 'Start Analysis' to begin fetching and analyzing data.
        
        **Note:**
        - Analysis may take a few minutes to complete
        - Using cached data by default
        - Enable 'Fetch Fresh Data' to get latest data
        """
        )
        return
    ...
    stocks_df["Total Trades"] = stocks_df["Symbol"].apply(
        lambda x: st.session_state.backtest_results.get(x, {}).get(
            "total_trades", 0
        )
    )
    ...
    stocks_df = stocks_df.sort_values(
        ["signal_order", "Total Trades", "Win Rate"],
